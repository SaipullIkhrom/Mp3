...
export default function Home() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-950 p-6 text-white selection:bg-blue-500/30">
      {/* Iklan Overlay Tetap Ada (Social Bar & Popunder) Jika ingin dihapus, hapus Script di bawah ini */}
      <Script
      ...
      <div className="w-full max-w-2xl space-y-8">
        ...
        {/* Input Section */}
        <div
          className={`group relative rounded-2xl border transition-all p-2 shadow-2xl backdrop-blur-xl ${
            mode === "youtube"
          ...
          {mode === "youtube" ? (
            ...
            <>
              ...
              <button
                onClick={() => handleDownload("mp3")}
                disabled={loading || !url}
                className="flex items-center justify-center gap-3 rounded-xl bg-slate-800 p-4 font-bold shadow-lg transition-all hover:bg-slate-700 active:scale-95 disabled:opacity-50"
              >
                <Music size={22} /> Download Mp3
              </button>
              ...
              <button
                onClick={() => handleDownload("mp4")}
                disabled={loading || !url}
                className="flex items-center justify-center gap-3 rounded-xl bg-linear-to-r from-pink-600 to-black p-4 font-bold shadow-lg transition-all hover:brightness-110 active:scale-95 disabled:opacity-50"
              >
                <Video size={22} /> Download Mp4
              </button>
            </>
            ...
            <div className="flex items-center justify-center gap-3 py-4">
              <div
                className={`h-6 w-6 animate-spin rounded-full border-2 border-t-transparent ${
                  mode === "youtube" ? "border-red-400" : "border-pink-400"
                }`}
              ></div>
              <span
                className={
                  mode === "youtube" ? "text-red-400" : "text-pink-400"
                }
              >
                Sedang memproses...
              </span>
        ...
        <footer className="mt-12 text-center pb-8 border-t border-white/5 pt-6">
          ...
          <p className="text-[10px] text-slate-600">
            © 2025 Yreaa Downloader. We are not affiliated with YouTube or
            TikTok.
          </p>
        </footer>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
            <div className="max-w-lg w-full bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl animate-in fade-in zoom-in duration-200">
              ...
              <div className="text-sm text-slate-300 space-y-3 overflow-y-auto max-h-[60vh] pr-2">
                <p>
                  <strong>1. Layanan:</strong> Menyediakan konversi video
                  YouTube dan TikTok untuk penggunaan pribadi.
                </p>
                <p>
                  <strong>2. Hak Cipta:</strong> Pengguna bertanggung jawab
                  penuh atas konten yang diunduh.
                </p>
                <p>
                  <strong>3. Iklan:</strong> Website didukung oleh iklan pihak
                  ketiga.
                </p>
              </div>
              <button
                onClick={() => setShowToS(false)}
                className="mt-6 w-full py-2 bg-slate-800 hover:bg-slate-700 rounded-lg font-bold"
              >
                Tutup
              </button>
            </div>
